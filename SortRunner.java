...
class SortRunner {
    ...
    // Run sorting algorithm and create .out file
    static void runAlgsPart1(int[] A, String infilename, Sorter alg) throws Exception {
    ...
    // Run sorting algorithm from 0 to n, and write CSV file
    static void runAlgsPart2(int[] A, String infilename, Sorter alg) throws Exception {
        ...
        String header = makeHeader(alg, digits);
        ...
        for (int i = 0; i <= A.length; i += INCREMENT) {
            // Construct the row, by running sorting algorithm
            String row = String.format(rowfmt, i);
            row += ", " + alg.runResetAndIncBy(INCREMENT);

            // Quit if sorting algorithm are discarded
    ...
    static String makeHeader(Sorter alg, int digits) {
        String header = alg.headerString();
        String fmt = "%" + digits + "s, %s";
        return String.format(fmt, "n", header);
    }
}
